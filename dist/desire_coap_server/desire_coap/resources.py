...
class ErtlResource(NodeResource):
    async def render_post(self, request: aiocoap.message.Message):
        ...
        rsp = aiocoap.Message(mtype=request.mtype,
                              code=aiocoap.numbers.codes.Code.CHANGED)
        ...
        try:
            if content_format == aiocoap.numbers.media_types_rev['application/json']:
                ...
                ertl = ErtlPayload.from_cbor_bytes(
...
class InfectedResource(NodeResource):
    ...
    async def render_get(self, request):
        ...
        content_format = request.opt.content_format
        try:
            infected_payload = InfectedPayload(
                self.handler.is_infected(self.node))
            if content_format == aiocoap.numbers.media_types_rev['application/json']:
    ...
    async def render_post(self, request: aiocoap.message.Message):
        ...
        try:
            if content_format == aiocoap.numbers.media_types_rev['application/json']:
                infected = InfectedPayload.from_json_str(
                    self.decrypt(request.payload))
...
class EsrResource(NodeResource):
    ...
    async def render_post(self, request: aiocoap.message.Message):
        ...
        try:
            if content_format == aiocoap.numbers.media_types_rev['application/json']:
                exposed = EsrPayload.from_json_str(
                    self.decrypt(request.payload))
                self.handler.set_exposed(self.node, exposed.contact)
                rsp.opt.content_format = content_format
            elif content_format == aiocoap.numbers.media_types_rev['application/cbor']:
                exposed = EsrPayload.from_cbor_bytes(
                    self.decrypt(request.payload))
                self.handler.set_exposed(self.node, exposed.contact)
                rsp.opt.content_format = content_format
            else:
...
class DesireCoapServer:
    ...
    def __post_init__(self):
        ...
        self.__coap_root.add_resource(['.well-known', 'core'],
                                      ...
                                      impl_info=None))
        self.__coap_root.add_resource(('.well-known', 'edhoc'),
                                      EdhocResource(edhoc_key.authcred,
                                                    edhoc_key.authkey, self.nodes))
        for node in self.nodes.nodes:
            self.__coap_root.add_resource([node.uid, 'ertl'], ErtlResource(
                node=node, handler=self.rq_handler))
            self.__coap_root.add_resource([node.uid, 'infected'], InfectedResource(
                node=node, handler=self.rq_handler))
    ...
    def run(self):
        if not self.host or not self.port:
            asyncio.Task(
                aiocoap.Context.create_server_context(self.__coap_root))
        else:
